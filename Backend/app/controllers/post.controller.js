...
const add_new_posts = (req, res) => {
  ...
  post.text = filter.clean(post.text); //to convert the bad words into asterisks
  console.log(post);
    ...
    if (err) res.send(err);
      ...
      posts.getSinglePost(post_id, (err, post) => {
        ...
        if (post.text === req.body.text) {
          ...
          let text = filter.clean(req.body.text);
    ...
    if (err) res.send(err);
      ...
      posts.getSinglePost(post_id, (err, post) => {
        ...
        if (err) return res.sendStatus(500);
          ...
          if (post.likes.some((like) => like.user_id === id)) {
            //.some and .find works the same, looks for the required condtion adn returns a boolean
